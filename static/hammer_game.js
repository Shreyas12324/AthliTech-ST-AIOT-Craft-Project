...
async function fetchSensorData() {
    try {
        ...
        let accelX = data.accel.x;
        let accelY = data.accel.y;
        let gyroX = data.gyro.x;
        let gyroY = data.gyro.y;
        let gyroZ = data.gyro.z;

        // ✅ Stop movement if accelY > 900
        if (accelY > 900) return;

        // ✅ Corrected MLC Prediction Key
        mlcPrediction = parseInt(data.mlc_prediction, 10);
        let gyroMovement = abs(gyroX) + abs(gyroY) + abs(gyroZ);

        // ✅ Apply correct mapping for Hammer Curl (X = 20 → 630, Y = -900 → 730)
        let mappedPosX = map(accelX, 20, 630, LOWER_BOUND, UPPER_BOUND);
        let mappedPosY = map(accelY, -900, 730, LOWER_BOUND, UPPER_BOUND);
        
        // ✅ Averaging mapped X and Y for smooth motion
        let smoothedPos = (mappedPosX + mappedPosY) / 2;
